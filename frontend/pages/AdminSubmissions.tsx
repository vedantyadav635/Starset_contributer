...
export const AdminSubmissions: React.FC = () => {
    ...
    const handleApprove = async (id: string) => {
        ...
        try {
            ...
            const res = await fetch(API_ENDPOINTS.APPROVE_SUBMISSION(id), {
                method: 'PATCH',
    ...
    const handleReject = async () => {
        ...
        try {
            const adminId = await getAdminId();
            const res = await fetch(API_ENDPOINTS.REJECT_SUBMISSION(rejectDialog.id), {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
    ...
    const statusBadge = (status: string) => {
        const map: Record<string, string> = {
            pending_validation: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400',
            approved: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400',
            rejected: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400',
            auto_passed: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400',
        };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-zinc-200 dark:border-white/10">
                <div>
                    <h1 className="text-2xl font-bold text-zinc-900 dark:text-white tracking-tight">Submission Review</h1>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                        ...
                        <select
                            ...
                            className="pl-9 pr-8 h-9 text-sm rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-600/20 appearance-none"
                        >
                    ...
                    <button
                        ...
                        className="h-9 w-9 flex items-center justify-center rounded-lg border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-white/5 transition-colors"
                    >
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {submissions.map(sub => (
                        <div
                            key={sub.id}
                            className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-white/10 shadow-sm overflow-hidden"
                        >
                            {/* Card Header */}
                            <div className="flex items-start justify-between p-5 pb-4 border-b border-zinc-100 dark:border-white/5">
                                <div className="flex items-start gap-4">
                                    <div className="h-10 w-10 bg-blue-50 dark:bg-blue-900/20 rounded-xl flex items-center justify-center flex-shrink-0">
                                        {sub.audio_url ? (
                            ...
                            <div className="p-5 grid md:grid-cols-2 gap-5">
                                ...
                                <div className="space-y-3">
                                    {sub.tasks?.prompt && (
                                        <div className="bg-zinc-50 dark:bg-white/5 rounded-xl p-4 border border-zinc-100 dark:border-white/5">
                                            ...
                                            <p className="text-sm text-zinc-800 dark:text-zinc-200 font-serif leading-relaxed">
                                                "{sub.tasks.prompt}"
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    {sub.text_content && (
                                        <div className="bg-zinc-50 dark:bg-white/5 rounded-xl p-4">
                                            ...
                                            <p className="text-sm text-zinc-700 dark:text-zinc-300">{sub.text_content}</p>
                                        </div>
                                    )}
                                    ...
                                    {sub.selected_option && (
                                        <div className="bg-zinc-50 dark:bg-white/5 rounded-xl p-4">
                                            ...
                                            <span className="inline-block px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-sm font-semibold rounded-lg">
                                                {sub.selected_option}
                                            </span>
                                        </div>
                                    )}
                                ...
                                <div className="space-y-3">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex gap-1 items-end h-8">
                                                        {[...Array(20)].map((_, i) => (
                                                            <div
                                                                key={i}
                                                                className={`w-1 rounded-full transition-all ${playingId === sub.id ? 'bg-blue-500 animate-pulse' : 'bg-zinc-300 dark:bg-zinc-700'
                                                                    }`}
                                    ...
                                    {sub.validation_errors && sub.validation_errors.length > 0 && (
                                        <div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-xl p-3">
                                            <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                                            <div>
                                                <p className="text-xs font-bold text-amber-700 dark:text-amber-400 mb-1">Auto-Validation Warnings</p>
                                    ...
                                    {sub.status === 'pending_validation' && (
                                        <div className="flex gap-3 pt-1">
                                            ...
                                            <Button
                                                ...
                                                disabled={!!processingId}
                                                variant="outline"
                                                className="flex-1 h-10 border-red-300 dark:border-red-900 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 text-sm"
                                            >
            ...
            {rejectDialog && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-white/10 shadow-2xl w-full max-w-md">
                        <div className="p-6">
                            <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-1">Reject Submission</h3>
                            ...
                            {rejectionReason === 'Other' && (
                                <textarea
                                    ...
                                    className="w-full p-3 rounded-xl border border-zinc-200 dark:border-white/10 bg-zinc-50 dark:bg-black/20 text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-red-500/20 resize-none h-20"
                                />
                            )}
